...
## Moss setup
The Moss connector is deliberately an adapter stub because the exact current Moss API/SDK contract must be taken from official Moss documentation/account. Set `MOSS_SEARCH_URL` and `MOSS_API_KEY` in environment, then adapt `agentguard/retrieval.py` to the actual authentication, request schema, response schema, and SDK. Never represent local baseline metrics as Moss measurements.
